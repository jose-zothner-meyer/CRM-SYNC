"""
Email Processor for CRM synchronization.

This processor:
1. Uses reliable Zoho API search methods
...
class EmailProcessor:
    """Email processor that handles CRM synchronization reliably"""
    ...
    def _create_note_safe(self, development_id: str, title: str, content: str) -> Dict:
        ...
        try:
            ...
            if self._is_success_response(result):
                ...
                error_msg = f"API returned failure: {result}"
                logger.error("Note creation failed: %s", error_msg)
                raise NoteCreationError(error_msg)
                
        except NoteCreationError:
            raise  # Re-raise specific exceptions
        except ZohoApiError as e:
            error_msg = f"Zoho API error during note creation: {str(e)}"
            logger.error(error_msg)
            return {'success': False, 'error': error_msg}
        except Exception as e:
            error_msg = f"Unexpected error during note creation: {str(e)}"
            logger.error(error_msg)
            raise NoteCreationError(error_msg) from e
...
"""
            ...
            logger.error("Note creation failed during fallback: %s", str(e))
            return {
                'success': False,
